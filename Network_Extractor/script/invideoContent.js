setTimeout(function () {
  ...
  function isForbidden() {
    ...
    if (document.location.protocol != "https:") return true; // Not Allowed
    // Only for chromecast links
    if (document.location.hostname.endsWith(".youtube.com")) return true; // Not Allowed
    if (document.location.hostname.endsWith("tiktok.com")) return true; // Not Allowed
    if (document.location.hostname.endsWith("instagram.com")) return true; // Not Allowed
    if (document.location.hostname.endsWith("vk.com")) return true; // Not Allowed
    if (document.location.hostname.endsWith("dailymotion.com")) return true; // Not Allowed
    return false;
  }
  ...
  function sendAllLinks() {
    ...
    if (lastUrl != url) {
      ...
      chrome.runtime.sendMessage({ msg: "msgSetUrl" }, function (response) {
        if (typeof response == "undefined" && chrome.runtime.lastError) return;
        if (response) {
          if (document.location.href.indexOf("vimeo.com") >= 0)
            findVimeoVideos(response.tabId);
          else scanPage(response.tabId);
        }
      });
    }
  }
  ...
  */
    ...
    for (var i = 0; i < document.links.length; i++) {
      ...
      if (u) {
        ...
        var cl = "";
        if (link.hasAttribute("class")) cl = myTrim(link.getAttribute("class"));
    ...
    for (var i = 0; i < a.length; i++) {
      ...
      if (u) {
        var title = "";
        if (link.hasAttribute("alt")) title = myTrim(link.getAttribute("alt"));
    ...
    if (tabId != -1) {
      ...
      if (allUrlsList.length > 0) fGreenArrow = true;
  
      if (!oembedUrl) oembedUrl = getOembedURL();
      if (oembedUrl != false) fGreenArrow = true;
  
      if (fGreenArrow) {
        chrome.runtime.sendMessage(
          { msg: "msgSetIcon", tabId: tabId, fVideo: true },
          function (response) {
  ...
  function OnAddToVideoList(info, play) {
    ...
    chrome.runtime.sendMessage(
      { msg: "msgAddToVideolist", info: info, url: location.href, play: play },
      function (response) {
        if (typeof response == "undefined" && chrome.runtime.lastError) return;
      }
    );
  ...
  */
  ...
  chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    if (request.id == "msgAddToList") {
      // Video gefunden von background.js
      var item = request.item;
      if (!item.title || item.title.length == 0) item.title = document.title;
    ...
    .catch(console.error);
                  ...
                  */
    } else if (request.id == "msgAddToVideolist") {
    ...
    /*else if (request.id == "SP24GetOEmbedUrl") {          
      ...
      if (!url) {
        sendResponse({
          info: false,
          error: "no oembed url available on " + location.href,
        });
        return;
      }
      ...
      getOembedInfo(url, function (response) {
        ...
        } else info = response; // Pass the error
        info.originalUrl = location.href;
        chrome.runtime.sendMessage(
          { msg: "msgOembedInfos", info: info },
          function (response) {
            if (typeof response == "undefined" && chrome.runtime.lastError)
              return;
          }
  ...
  function FindFirstUrl(html, ext, start) {
    for (;;) {
      ...
      if (s.indexOf("/") == 0) return { mp4: server + s, start: i1 };
      if (s.indexOf("\\/") == 0) {
        s = s.replace(/\\\//g, "/");
    ...
    xmlHttpReq.onreadystatechange = function (data) {
      ...
      if (i >= 0) {
        ...
        for (var j = i; j + 1 < s.length; j++) {
          if (s.charAt(j) == "\\")
            // Backslash vor Gänsefüßchen zählt nicht
            j++;
